...
def get_course_code(course_name, codes_data, actual_courses_data):
    ...
    possible_codes = [row[1] for row in codes_data if row[0].strip().lower() == course_name]
...
def update_schedule(schedule_file, codes_file, actual_courses_file):
    ...
    for row in schedule_data[1:]:
        updated_row = [row[0]]  # Keep the day column unchanged
        for cell in row[1:]:
            cell = cell.strip()
            ...
            updated_courses = [f"{course} ({get_course_code(course, codes_data, actual_courses_data)})" for course in courses]

            formatted_cell = " & ".join(updated_courses)
            # Apply quoting only when necessary
            if len(courses) > 1:
                formatted_cell = f'"{formatted_cell}"'

            updated_row.append(formatted_cell)

        updated_rows.append(updated_row)

    output_file = "updated_" + os.path.basename(schedule_file)
    with open(output_file, "w", newline="") as csvfile:
        writer = csv.writer(csvfile)
        writer.writerows(updated_rows)

if __name__ == "__main__":
    update_schedule("ema_schedule_copy.csv", "codes.csv", "actual_codes.csv")
