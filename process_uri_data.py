...
df_concurrencia = pd.read_csv("data/matriz_concurrencia.csv", dtype={'nom_periodo': str})
df_inscripciones = pd.read_csv("data/inscritos_por_uc_anio.csv")

# Procesar matriz de concurrencia
...
mapping_df = mapping_df.sort_values("descripcion")
...
avg_concurrencia = df_concurrencia.groupby(["cod1", "cod2", "semestre"])["cantidad"].mean().reset_index()
...
avg_conc_sem1 = avg_conc_sem1.rename(columns={
    'cod1': 'uc_1',
    'cod2': 'uc_2',
    'cantidad': 'coincidencia'
}).drop('semestre', axis=1)

avg_conc_sem2 = avg_conc_sem2.rename(columns={
    'cod1': 'uc_1',
    'cod2': 'uc_2',
    'cantidad': 'coincidencia'
}).drop('semestre', axis=1)

# Procesar inscripciones por semestre
...
df_inscripciones['semestre'] = df_inscripciones['fecha_inicio'].dt.month.map(lambda x: 1 if x <= 6 else 2)

# Calcular promedio por código y semestre
avg_inscripciones = df_inscripciones.groupby(
    ['codenservicio_mat', 'semestre']
...
avg_insc_sem1 = avg_insc_sem1.rename(columns={
    'codenservicio_mat': 'uc',
    'cantidad': 'inscriptos'
}).drop('semestre', axis=1)

avg_insc_sem2 = avg_insc_sem2.rename(columns={
    'codenservicio_mat': 'uc',
    'cantidad': 'inscriptos'
}).drop('semestre', axis=1)
